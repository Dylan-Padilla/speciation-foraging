---
title: "Data analysis for: Relative High Fitness and Large Genome Size May Lead to the High Diversification of Plastic Foragers"
...
par(mar = c(5, 3.9, 1, 0.5), col.lab = 'black', cex.lab = 1.4)
plot(NA, ylim = c(0, 70), xlim = c(0, 0.30),
     axes = FALSE, ylab = '', xlab = '')
grid()
par(new = TRUE)
profiles.plot(net.div,
              ...
              col.line = setNames(colors, colnames(net.div)),
              lty = 1)
legend('topleft', legend = c(expression(lambda[1]-mu[1]),
                              expression(lambda[2]-mu[2]),
                              expression(lambda[3]-mu[3])),
       fill = colors, bty = 'n', cex = 1.4)

mtext('C', at = -0.36, side = 3, line = 2.1)
...
lifetime.repro.model1 <- gls(lifetime.repro.output ~ Foraging.mode*body.mass,
                     correlation =
...
lifetime.repro.model2 <- gls(lifetime.repro.output ~ Foraging.mode + body.mass, 
                     correlation =
                         corBrownian(phy = tree.life.rep,
                                     form = ~Species),
                     data = data.lifetime.rep.out, method = "ML")
...
lifetime.repro.model3 <- gls(lifetime.repro.output ~ Foraging.mode,
                     correlation =
                         corBrownian(phy = tree.life.rep,
                                     form = ~Species),
                     data = data.lifetime.rep.out, method = "ML")
...
lifetime.repro.model4 <- gls(lifetime.repro.output ~ body.mass,
                     correlation =
                         corBrownian(phy = tree.life.rep,
                                     form = ~Species),
                     data = data.lifetime.rep.out, method = "ML")
...
pred.mod <- data.frame(body.mass = seq(min(data.lifetime.rep.out$body.mass),
                                       max(data.lifetime.rep.out$body.mass), len = 246),
...
active <- pred.mod[data.lifetime.rep.out$Foraging.mode == 'active', ]
plastic <- pred.mod[data.lifetime.rep.out$Foraging.mode == 'plastic', ]
sit.and.wait <- pred.mod[data.lifetime.rep.out$Foraging.mode == 'sit-and-wait', ]
...
head(pruned.geno.size.data[ , c(1:2, 13, 15, 30)])
...
cost.model1 <- gls(lifetime.rep.out ~ Foraging.mode*genome.size*max.body.mass,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                 data = pruned.geno.size.data, method = "ML")

cost.model2 <- gls(lifetime.rep.out ~ Foraging.mode*genome.size + max.body.mass,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                 data = pruned.geno.size.data, method = "ML")

cost.model3 <- gls(lifetime.rep.out ~ Foraging.mode*genome.size,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                 data = pruned.geno.size.data, method = "ML")
...
cost.model4 <- gls(lifetime.rep.out ~ Foraging.mode + genome.size + max.body.mass,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                     data = pruned.geno.size.data, method = "ML")
...
cost.model5 <- gls(lifetime.rep.out ~ Foraging.mode,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                 data = pruned.geno.size.data, method = "ML")
...
cost.model6 <- gls(lifetime.rep.out ~ genome.size,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                     data = pruned.geno.size.data, method = "ML")

cost.model7 <- gls(lifetime.rep.out ~ max.body.mass,
                 correlation =
                         corBrownian(phy = geno.size.tree,
                                     form = ~Species),
                     data = pruned.geno.size.data, method = "ML")
...
pred.mod <- data.frame(max.body.mass = seq(min(pruned.geno.size.data$max.body.mass),
                                       ...
                                       len = 57),
                       genome.size = seq(min(pruned.geno.size.data$genome.size),
...
pred.mod$lifetime.rep.out <- predict(cost.model4, pred.mod,
                          type = 'response', se = TRUE)$fit
pred.mod$se <- predict(cost.model1, pred.mod,
                       type = 'response', se = TRUE)$se.fit
...
plot(lifetime.rep.out ~ genome.size, data = pruned.geno.size.data,
...
lines(active$lifetime.rep.out ~ active$genome.size,
      data = active, lwd = 2.5, col = '#39568CFF')

lines(plastic$lifetime.rep.out ~ plastic$genome.size,
      data = plastic, lwd = 2.5, col = 'green4')

lines(sit.and.wait$lifetime.rep.out ~ sit.and.wait$genome.size,
      data = sit.and.wait, lwd = 2.5, col = '#DCE319FF')
...
legend('topleft',
       ...
       col = c('#39568CFF', 'green4', '#DCE319FF'), lwd = 1,
       bty = 'n', cex = 0.8)
mtext('A', side = 2, at = 3.7, las = 2, line = 3.1)
...
plot(lifetime.rep.out ~ max.body.mass, data = pruned.geno.size.data,
     pch = 21, bg = cols[as.numeric(as.factor(pruned.geno.size.data$Foraging.mode))],
     las = 1, xaxt = 'n', yaxt = 'n', xlab = '', ylab = '',
     type = 'n', axes = FALSE)
...
lines(active$lifetime.rep.out ~ active$max.body.mass,
      data = active, lwd = 2.5, col = '#39568CFF')

lines(plastic$lifetime.rep.out ~ plastic$max.body.mass,
      data = plastic, lwd = 2.5, col = 'green4')

lines(sit.and.wait$lifetime.rep.out ~ sit.and.wait$max.body.mass,
      data = sit.and.wait, lwd = 2.5, col = '#DCE319FF')
...
legend('topleft',
       ...
       col = c('#39568CFF', 'green4', '#DCE319FF'), lwd = 1,
       bty = 'n', cex = 0.8)
mtext('B', side = 2, at = 3.7, las = 2, line = 3.1)
