---
title: "Data analysis for: Relative High Fitness and Genome-wide Diversity May Facilitate Plastic and Active Foragers' Diversification"
...
##png('./imgs/figureS2.png', width = 7, height = 7, units = 'in',
##    res = 360)
...
plot(NA, xlim = range(times), ylim = c(0, max(log1p(lineages))),
     ylab = expression('Lineages'~(log[1+p])), bty = "n", las = 1,
     xlab = "Time (mya)", axes = FALSE)
grid()
...
axis(1, at = round(seq(0, max(nodeHeights(lizard.maps.sym[[idx]])),
     ...
     label = round(seq(max(nodeHeights(lizard.maps.sym[[idx]])), 0,
                       length.out = 4), 0), cex.axis = 0.8)
axis(2, las = 1, cex.axis = 0.8)
mtext('A', side = 2, at = 7, las = 2, line = 3)
box()
...
## computing genome size standard deviation among foraging behavior categories

tapply(pruned.geno.size.data$genome.size,
       list(pruned.geno.size.data$Foraging.mode), sd)
